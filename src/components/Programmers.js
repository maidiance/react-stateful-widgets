...
export default function Programmers() {
  ...
  const [programmerList, setList] = useState(listOfAwesome);
  ...
  const getNameOfFeatured = () => {
    ...
    return programmerList[featuredProgrammer - 1].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (featuredProgrammer === '') ? 'royalblue' : 'gold', // 🤔 color turns to gold, when celebrating
  };
